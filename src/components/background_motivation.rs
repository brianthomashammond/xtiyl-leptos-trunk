...
#[component]
pub fn BackgroundMotivation() -> impl IntoView {
    let state = use_context::<RwSignal<GlobalState>>()
        .expect("state to have been provided");

    let background = create_memo(move |_| state.get().background);

    let background_motivation = match background() {
        "Acolyte" => {
            match roll_die(6) {
    ...
    view! {
        <div>
            <p>"Background Motivation: "{background_motivation}</p>
        </div>
    }
}
